...
const CommunityOnboarding: React.FC = () => {
  ...
  const finish = () => {
    // Mark profile complete so the user can view other alumni from here on.
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('forge.profileComplete', 'true');
    }
    navigate('/community-redesign');
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black p-4 sm:p-6 text-foreground font-sans antialiased">
      ...
      <div className="relative flex h-[min(760px,calc(100dvh-2rem))] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-border/40 bg-card shadow-[0_30px_80px_-20px_rgba(0,0,0,0.8)]">
        ...
        <TopBar
          onExit={() => navigate('/community-redesign')}
...
// ---------- Top bar ----------
const TopBar: React.FC<{ onExit: () => void; showProgress: boolean; current: number; total: number; required?: boolean }> = ({ onExit, showProgress, current, total, required }) => (
  <header className="flex items-center justify-between gap-3 border-b border-border/40 px-6 sm:px-8 py-4">
    <div className="flex items-center gap-3">
      ...
      {required && (
        <span className="hidden sm:inline-flex items-center gap-1.5 rounded-full border border-primary/40 bg-primary/10 px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] text-primary">
          <span className="h-1.5 w-1.5 rounded-full bg-primary animate-pulse" /> Required to continue
        </span>
      )}
    </div>
    {showProgress ? (
    ...
    {required ? (
      <span className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground/60 hidden sm:inline">
        Cannot skip
      </span>
    ) : (
    <button
...
const WelcomeStep: React.FC<{ onStart: () => void }> = () => (
  <div className="flex flex-col items-center text-center">
    ...
    <h1 className="font-bold mt-4 text-[40px] sm:text-[48px] leading-[0.95] tracking-tight text-foreground">
      Welcome to
      <br />
...
const ReviewStep: React.FC<{ data: FormData; onEdit: (idx: number) => void }> = ({ data, onEdit }) => (
  <div>
    ...
    <h1 className="font-bold mt-2 text-[32px] sm:text-[38px] leading-[1.02] tracking-tight text-foreground">
...
const StepLayout: React.FC<{ eyebrow: string; title: React.ReactNode; subtitle: string; children: React.ReactNode }> = ({ eyebrow, title, subtitle, children }) => (
  <div className="space-y-6">
    <div>
      ...
      <h1 className="font-bold mt-2 text-[32px] sm:text-[38px] leading-[1.02] tracking-tight text-foreground">{title}</h1>
