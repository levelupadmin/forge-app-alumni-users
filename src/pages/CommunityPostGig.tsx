...
const CommunityPostGig: React.FC = () => {
  ...
  const publish = () => {
    if (!isValid) return;
    setPublished(true);
    setTimeout(() => navigate('/community-redesign'), 1600);
  };
          ...
          <footer className="border-t border-border/40 bg-card">
            <div className="flex items-center justify-between gap-3 px-6 sm:px-8 py-4">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  Publish gig <Check className="h-4 w-4" />
                </button>
              </div>
            </div>
          </footer>
...
const LivePreview: React.FC<{ data: FormData }> = ({ data }) => (
  <div className="p-6 space-y-5">
    ...
    <article className="rounded-2xl border border-border/40 bg-background/40 p-5">
      ...
      <h3 className={cn(
        'mt-2.5 text-xl font-bold leading-tight',
...
const PublishedSuccess: React.FC<{ data: FormData }> = ({ data }) => (
  <div className="flex h-full flex-col items-center justify-center text-center">
    ...
    <h2 className="font-bold mt-5 text-[32px] leading-[1.02] tracking-tight text-foreground">
